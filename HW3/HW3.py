...
ax.plot(s_hat[:, 1],'r--', linewidth=2, label='Original')
ax.plot(y_hat_1[:, 0], linewidth=1, label='Reconstructed')
ax.legend(["Original", "Reconstructed"])
ax1.plot(s_hat[:, 0], 'r--', linewidth=2, label='Original')
ax1.plot(y_hat_1[:, 1], linewidth=1, label='Reconstructed')
ax1.legend(["Original", "Reconstructed"])
plt.show()
